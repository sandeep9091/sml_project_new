...
class LoanApprovalsActionPage extends StatelessWidget {
  const LoanApprovalsActionPage({super.key,this.eachGeneratedLoan,required this.type});
  final GenerateLoansResponseData? eachGeneratedLoan;
  final String type;

  @override
  Widget build(BuildContext context) {
    LoanApprovalsPageViewModel model = ProviderScope.containerOf(context).read(loanApprovalsViewModelProvider);
    ...
    return Scaffold(
      ...
      child: Container(
        ...
        child: Column(children: [
          ...
          ValueListenableBuilder(
                valueListenable: model.selectedLoanStatus,
                builder: (BuildContext context, selectedValue, child) {
                  return AppDropDownWidget<dynamic>(
                      labelText: "Loan Status",
                      dropdownHeight: 44,
                      hintText: "Select Loan Status",
                      borderRadius: 6.0,
                      buttonBroderColor: AppColor.grey,
                      isMandatory: true,
                      isEnabled: true,
                      dropDownitems: CommonLists.loanStatusList.map(
                          (e) => DropdownMenuItem<dynamic>(
                            value: e['value']??"",
                            child: Text(e['label']??""),
                          ),
                        ).toList(),
                      selectedValue: (model.selectedLoanStatus.value != "" &&
                              CommonLists.loanStatusList.any((element) => element['value'] == model.selectedLoanStatus.value))
                          ? model.selectedLoanStatus.value
                          : null,
                      onChangedMethod: (newValue) {
                        model.selectedLoanStatus.value = newValue;
                      });
                }),
          AppTextField(
                labelText: "Loan Status Remarks",
                inputType: TextInputType.multiline,
                inputFormatters: [FilteringTextInputFormatter.allow(RegExp("[a-zA-Z0-9 ]"))],
                borderRadius: 5,
                maxLines: 10,
                controller: model.controllerLoanStatusRemarks,
                padding: const EdgeInsets.symmetric(vertical: 2,horizontal: 5),
                enabled: true,
                onChanged: (String value){
                },             
          ...
          dataBuilder: ((context, data) {
            ...
            isEnable: true,
            onPressed: (){
              if(model.selectedLoanStatus.value !=""){
                 model.saveGenerateLoanData(eachGeneratedLoan);
              }else{
                model.showToastWithString("Loan Status is missing");
              }
            },);
          }),
        ),
