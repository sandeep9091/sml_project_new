...
class GenerateLoansActionPage extends StatelessWidget {
  const GenerateLoansActionPage({super.key,this.eachGeneratedLoan,required this.type});
  final GenerateLoansResponseData? eachGeneratedLoan;
  final String type;

  @override
  Widget build(BuildContext context) {
    GenerateLoansPageViewModel model = ProviderScope.containerOf(context).read(generateLoansViewModelProvider);
    ...
    return Scaffold(
      appBar: const CommonAppBar(
        enalbeTitle: true,
        titleWidget: Text("Generate Loan"),
      ),
      ...
      child: Container(
        ...
        child: Column(children: [
          ...
          dataBuilder: ((context, data) {
            ...
            onPressed: (){
              if(model.selectedBorrowerName.value.isNotEmpty && model.selectedHouseType.value !=0 &&
              model.selectedProductType.value !=0 && model.selectedLoanAmount.value !=0 &&
                model.selectedRepayMode.value !=0 && model.selectedLoanTenure.value !=0 &&
                model.selectedSurityHouseType.value !=0 && model.controllerbank.text.isNotEmpty &&
                model.controllerAccountNo.text.isNotEmpty && model.controllerAccountName.text.isNotEmpty &&
                model.controllerIfscCode.text.isNotEmpty && model.controllerSurityName.text.isNotEmpty &&
                model.controllerSurityAadhar.text.isNotEmpty && model.controllerSurityContactNo.text.isNotEmpty
                ){
                 model.saveGenerateLoanData();
              }else{
